...
- Fee endpoints enforce tenant access, track latency histograms, and now validate replay-resistant HMAC signatures (timestamp, nonce, persisted nonce store) alongside the existing in-memory rate limiter. 【F:backend/app/routers/fees.py†L1-L214】【F:backend/app/security/hmac.py†L1-L161】【F:backend/app/security/rate_limit.py†L1-L46】
...
## Observability
- Prometheus counters and histograms exist for fee decisions, report exports, auth events, analytics dashboard loads, and security failures/replays; observability docs describe these signals. 【F:backend/app/observability.py†L5-L90】【F:docs/observability.md†L1-L150】
...
- UI guide documents analytics dashboard behavior, session metadata, and export flows. 【F:docs/ui-guide.md†L1-L80】
...
## Divergences & Gaps
- Per-store rate limiting remains in-memory; milestone plan should outline the Redis-backed implementation and observability before rollout. 【F:backend/app/security/rate_limit.py†L1-L46】【F:docs/backlog/milestone_04_security.md†L61-L120】
- Frontend surfaces the last HMAC rotation timestamp but still lacks a rotation action/secret reveal workflow. 【F:src/pages/Settings.tsx†L1-L260】【F:docs/security/hmac.md†L1-L120】
- CI/operations documentation should define how to supply `SMOKE_HMAC_SECRET`/`hmac_secret` secrets in non-demo environments to avoid hard-coded defaults. 【F:README.md†L80-L110】【F:docs/postman/README.md†L1-L140】
...
- Advance to **Milestone 4 – Security & Rate Limiting** by tackling distributed rate limiting, secret rotation workflows, and CI/operations guidance now that replay-resistant HMAC validation and telemetry are in place.
