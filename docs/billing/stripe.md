...
1. Create recurring products in the Stripe Dashboard for cada tier: **Free/Dev** (opcional), **Starter**, **Pro**, **Plus**, **Enterprise 10k**, **Enterprise 25k**, **Enterprise 50k**.
2. Crie os preços mensais correspondentes e capture os price IDs.
3. Popule as seguintes variáveis de ambiente (ver `.env.example` para placeholders):
   ...
   STRIPE_PRICE_ID_E10K=price_...
   STRIPE_PRICE_ID_E25K=price_...
   STRIPE_PRICE_ID_E50K=price_...
   ```
...
4. **Validação rápida**
   ...
   ```bash
   make billing-smoke
   # Esperado: resumo de limites + "Billing smoke completed successfully."
   ```
